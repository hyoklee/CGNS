...
# Parallel benchmark creates timing_NNNNNN_MODE.dat files
TIMING_FILE=$(ls timing_*.dat 2>/dev/null | head -1)
if [ -n "$TIMING_FILE" ]; then
  echo "Timing data for 32K elements:" | tee -a benchmark_output.txt
  cat "$TIMING_FILE" | tee -a benchmark_output.txt
  cp "$TIMING_FILE" timing_32k.dat
  rm -f timing_*.dat  # Clean up for next test
fi
echo "" | tee -a benchmark_output.txt
...
if [ -n "$TIMING_FILE" ]; then
  ...
  cat "$TIMING_FILE" | tee -a benchmark_output.txt
  cp "$TIMING_FILE" timing_128k.dat
  rm -f timing_*.dat
fi
echo "" | tee -a benchmark_output.txt
...
if [ -n "$TIMING_FILE" ]; then
  ...
  cat "$TIMING_FILE" | tee -a benchmark_output.txt
  cp "$TIMING_FILE" timing_512k.dat
  rm -f timing_*.dat
fi
echo "" | tee -a benchmark_output.txt
